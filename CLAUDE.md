...
- Verify with `pnpm -r lint && pnpm -r typecheck && pnpm -r test` before finishing any task. Integration: `pnpm --filter @loadless/api test:integration`. Browser e2e: `bash apps/web/e2e/prepare.sh && (cd apps/web && pnpm e2e)` — 43 tests covering every flow; specs live in `apps/web/e2e/`.
